...
const { default: pins } = await import('./alchemy-item-pins.js');

const styleCount = () => document.querySelectorAll('#mwi-alchemy-pins-style').length;
...
describe('a character switch landing inside the stored-pins read', () => {
    beforeEach(() => {
        ...
        pins.disable();
        observerMock.live = 0;
        document.querySelectorAll('#mwi-alchemy-pins-style').forEach((el) => el.remove());
    });
